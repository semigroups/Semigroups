...
def _check_package_info():
    ...
    if match:
        ...
        exit_abort('Can\'t find the semigroups version for the doc in PackageInfo.g')

    # check the ArchiveURL
    match = re.compile(r'ArchiveURL\s+:=\s+"([^"]*)"').search(contents)
    if match:
        if not match.group(1).endswith('semigroups-' + _VERSION):
            exit_abort('The ArchiveURL is ' + match.group(1).split('/')[-1]
                       + ' should be semigroups-' + _VERSION)
        else:
...
def _main():
    ...
    tmpdir = tmpdir_base + '/semigroups-' + _VERSION
    ...
    if args.skip_tests:
        ...
        for directory in args.gap_root:
            semigroups_dir = os.path.join(directory, 'pkg/semigroups-' + _VERSION)
            try:
                shutil.copytree('semigroups-' + _VERSION, semigroups_dir)
    ...
    os.chdir(tmpdir_base)
    exec_string('tar -cpf semigroups-' + _VERSION + '.tar semigroups-' + _VERSION)
    exec_string('gzip -9 semigroups-' + _VERSION + '.tar')

    # Copy files to the local copy of the website
    info_action('Copying to webpage' + ' . . .')
    try:
        os.chdir(tmpdir_base)
        shutil.copy('semigroups-' + _VERSION + '.tar.gz', _WEBPAGE_DIR)
        os.chdir(tmpdir)
    ...
    if not args.dry_run:
        ...
        exec_string('hg add semigroups-' + _VERSION + '.tar.gz')
        exec_string('hg addremove')
        info_action('Committing webpage repo')
        exec_string('hg commit -m "Releasing semigroups ' + _VERSION + '"')
