...
echo "LoadPackage(\"$PACKAGE\", false); SemigroupsTestInstall(); Test(\"save-workspace.tst\"); quit; quit; quit;" | $GAPDIR/bin/gap.sh -A -r -m 1g -T 2>&1 | tee -a $TESTLOG
echo "Test(\"load-workspace.tst\"); SemigroupsTestInstall(); quit; quit; quit;" | $GAPDIR/bin/gap.sh -L test-output.w -A -r -m 1g -T 2>&1 | tee -a $TESTLOG
rm test-output.w
cd ../..

# Run all tests and manual examples
echo -e "\nRunning tests and manual examples..."
echo "LoadPackage(\"$PACKAGE\"); SemigroupsTestStandard(); SEMIGROUPS.TestManualExamples(); quit; quit; quit;" | $GAPDIR/bin/gap.sh -A -r -m 1g -T 2>&1 | tee $TESTLOG
