  ...
  public:
    ...
    void closure (const Semigroup<T>*     old,
      ...
      for (size_t i = 0; i < irr_coll.size(); i++) {
        ...
        _first.push_back(_gens.size() + i);
        _final.push_back(_gens.size() + i);
      ...
      for (size_t i = 0; i < old_new.size(); i++) {
        if (!old_new.at(i)) {
          tmp.push_back(i);
        }
      }
