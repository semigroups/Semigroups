...
class Semigroup : public SemigroupBase {
  ...
  public:
    ...
    Semigroup (Semigroup<T>* old, const std::vector<T*>& coll, size_t deg, bool report) :
        ...
        _pos_one    (0), 
        _prefix     (), 
        _reduced    (RecVec<bool>(old->nrgens() + coll.size(), old->_nr)),
        _left       (new RecVec<size_t>(old->nrgens() + coll.size(), old->_nr)),
        _right      (new RecVec<size_t>(old->nrgens() + coll.size(), old->_nr)),
        _suffix     (), 
        _wordlen    (0) // (length of the current word) - 1
    {
      _elements->reserve(old->_nr);
      _final.reserve(old->_nr);
      _first.reserve(old->_nr);
      _map.reserve(old->_nr);
      _prefix.reserve(old->_nr);
      _suffix.reserve(old->_nr);
