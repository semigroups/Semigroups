...
# IsMonogenicSemigroup, IsLeftCancellative, 
...
function(s)
  ...
  if not IsGroupAsSemigroup(I) then
    Info(InfoWarning, 2, "minimal ideal is not a group.");
    return false;
  elif not IsCyclic(Range(IsomorphismPermGroup(I))) then 
    Info(InfoWarning, 2, "minimal ideal is not a cyclic group.");
    return false;
  fi;

  max:=[0,0]; index:=EmptyPlist(m); j:=0;
  
  for i in [1..m] do 
    x:=IndexPeriodOfTransformation(gens[i]);
    if Sum(x)>max then 
      max:=x; pos:=i;
    fi;
    if x[2]=Size(I) then 
  ...
  if index=[] then 
    Info(InfoWarning, 2, "minimal ideal is not contained in the semigroup", 
    " generated by any");
    Info(InfoWarning, 2, "single generator.");
    return false;
  fi;

  pos:=List(Filtered(index, x-> x[2]=max), x-> x[1]);
