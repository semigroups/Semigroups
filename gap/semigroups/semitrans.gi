...
function(T)
  ...
  if d = 1 then
    for i in [1 .. DigraphNrEdges(O) + 1] do
      Ext := ExtRepOfObj(N[2 * i]);
      M := [1 .. Size(Ext)];
      for j in [1 .. Size(Ext)] do
        M[j] := ShallowCopy(Ext[j]);
      od;
      for j in [1 .. Size(Ext)] do
        for k in [1 .. Size(Ext[j])] do
          M[j][k] := - Ext[j][k];
        od;
      od;
  ...
  for gcount in [1 .. Size(SmallGeneratingSet(G))] do
    N[Size(N) - gcount + 1] := AsBipartition(SmallGeneratingSet(G)[gcount], n);
  od;
  return N;
  end);
