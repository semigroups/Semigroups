...
# the object containing <GeneratorsOfSemigroup> which are used to compute the
# ideal. For a regular semigroup ideal, <SupersemigroupOfIdeal> will always be
# the top most semigroup used to create any of the predecessors of the current
# ideal. For example, if <S> is a semigroup, <I> is a regular ideal of <S>, and
# <J> is an ideal of <I>, then <Parent(J)> is <I> and <SupersemigroupOfIdeal(J)>
