...
###############################################################################
# LatticeOfCongs functions
###############################################################################
#
# These abstract functions takes a semigroup 'S', a string 'type_string' and a
# record 'record'.
...
# record may contain either of the following components, which should be set to
...
#
# The basic LatticeOfCongs function returns a lattice of all the appropriate
# x-congs, while LatticeOfCongs_min returns only the minimal x-congs and
# LatticeOfCongs_1gen returns only those generated by a single pair (not
# including the trivial congruence).
#
###############################################################################
SEMIGROUPS.LatticeOfCongs := function(S, type_string, record)
  local latt;
  # Find the lattice of 1-generated congs
  latt := SEMIGROUPS.LatticeOfCongs_1gen(S, type_string, record);

  # Take all the joins it generates
  return SEMIGROUPS.JoinLatticeOfCongs(S, latt, type_string, record);
end;

#

SEMIGROUPS.LatticeOfCongs_1gen := function(S, type_string, record)
  local report, transrep, _XSemigroupCongruence, elms, pairs, total, congs,
        nrcongs, children, parents, last_collected, nr, pair, badcong,
        newchildren, newparents, newcong, i, c, p, orig_congs, 2congs, image,
        next, lattice;

  if not IsFinite(S) then
    ErrorNoReturn("Semigroups: SEMIGROUPS.LatticeOfCongs: usage,\n",
...
SEMIGROUPS.LatticeOfCongs_min := function(S, type_string, record)
...
###############################################################################
#
# This function takes a semigroup <S> and a congruence lattice <latt> and
# returns the lattice of all congruences which can be found by taking arbitrary
# joins in <latt>.  The <type_string> and <record> arguments are the same as in
# the LatticeOfCongs functions above.
#
###############################################################################
SEMIGROUPS.JoinLatticeOfCongs := function(S, latt, type_string, record)
  local report, congs1, congs, children, parents, nrcongs, transrep, 2congs,
        _XSemigroupCongruence, join_func, length, found, ignore, start, i, j,
        newcong, badcong, newchildren, newparents, k, p, c, image, next,
        set_func, lattice;
...
S -> SEMIGROUPS.LatticeOfCongs(S, "Left", rec()));

InstallMethod(LatticeOfRightCongruences,
"for a semigroup", [IsSemigroup],
S -> SEMIGROUPS.LatticeOfCongs(S, "Right", rec()));

InstallMethod(LatticeOfCongruences,
"for a semigroup", [IsSemigroup],
S -> SEMIGROUPS.LatticeOfCongs(S, "", rec()));
...
function(S, restriction)
  ...
  return SEMIGROUPS.LatticeOfCongs(S, "Left", rec(restriction := restriction));
end);
...
function(S, restriction)
  ...
  return SEMIGROUPS.LatticeOfCongs(S, "Right", rec(restriction := restriction));
end);
...
function(S, restriction)
  ...
  return SEMIGROUPS.LatticeOfCongs(S, "", rec(restriction := restriction));
end);
...
"for a semigroup", [IsSemigroup],
S -> SEMIGROUPS.LatticeOfCongs_min(S, "Left", rec())![1]);

InstallMethod(MinimalRightCongruencesOfSemigroup,
"for a semigroup", [IsSemigroup],
S -> SEMIGROUPS.LatticeOfCongs_min(S, "Right", rec())![1]);

InstallMethod(MinimalCongruencesOfSemigroup,
"for a semigroup", [IsSemigroup],
S -> SEMIGROUPS.LatticeOfCongs_min(S, "", rec())![1]);
