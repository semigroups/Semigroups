...
function(s, coll, opts)
  local t, old_data, n, max_rank, orbits, lens, data_ht, data, data_len, images, old_lens, old_orbits, gens, ht, o, r, j, scc, reps, out, old_reps, old_data_list, old_reps_len, old_o, new_data, d, g, m, z, old_index, old_gen1, old_gen2, old_pos1, old_pos2, gen1, pos1, gen2, pos2, i, k, val, y;
  ...
  if not HasOrbitsOfImages(s) then 
    Info(InfoCitrus, 2, "Nothing known about the old semigroup.");
    return t;
  fi;
  ...
  #schreier###############################################################
    ...
    gens:=List(gens, ShallowCopy);
    #JDM remove previous line in future if Orb is fixed.

    r:=CitrusHashLen!.rclassreps_orb;
    o:=EmptyPlist(r);
    ht := HTCreate([1..n]*1, rec(forflatplainlists:=true, hashlen:=r));
    ht!.o:=o;
    j:=HTAdd(ht, [1..n]*1, 1);
    o[1]:=ht!.els[j]; 
          ...
          for m in [1..Length(scc)] do
            for val in [1..Length(reps[m])] do
              for n in [1..Length(reps[m][val])] do
                ...
                HTAdd(data_ht, out, data_len);
                data[data_len]:=out; 
                old_index[data_len]:=HTValue(old_data!.data_ht, out);
              od;
            od;
          od;  
          for i in o do 
    ...
    # install predecessors of R-reps in orbit

    for i in [1..Length(old_index)] do 
      if old_pos1[old_pos2[old_index[i]]]=fail then 
        r:=r+1;
        pos2[i]:=r;
        gen2[i]:=old_gen2[old_index[i]];
        j:=HTAdd(ht, old_o[old_pos2[old_index[i]]], r);
        o[r]:=ht!.els[j];
      else
        pos2[i]:=HTValue(ht, old_o[old_pos2[old_index[i]]]);
        gen2[i]:=old_gen2[old_index[i]];
      fi;
    od;

    # install successors of R-reps in orbit
    ...
    new_data:= Objectify(NewType(FamilyObj(t), IsOrbitsOfImages), 
     rec(finished:=false, orbits:=orbits, lens:=lens, images:=images,
      at:=1, gens:=gens, ht:=ht, data_ht:=data_ht, data:=data));
