...
SEMIGROUPS.InitCCSemigroupData := function(S)

  Assert(1, SEMIGROUPS.IsCCSemigroup(S));
...
SEMIGROUPS.InitHashableSemigroupData := function(S)
  local data, hashlen, nrgens, nr, val, i;

  Assert(1, not SEMIGROUPS.IsCCSemigroup(S));
  Assert(1, ChooseHashFunction(Representative(S), 1) <> fail);
  Assert(1, Length(GeneratorsOfSemigroup(S)) > 0);
...
SEMIGROUPS.InitNonHashableSemigroupData := function(S)
  local data, hashlen, nrgens, nr, val, i;
  ...
  # add the generators
  for i in data.genstoapply do
    val := Position(data.set, data.gens[i]);
    if val = fail then # new generator
      nr := nr + 1;
      AddSet(data.set, data.gens[i]);
      data.elts[nr] := data.gens[i];
      data.words[nr] := [i];
      data.first[nr] := i;
      data.final[nr] := i;
