...
function(L)
  ...
  if IsRegularClass(L) or Length(RhoCosets(D)) = 1 then
    ...
    return [One(SchutzenbergerGroup(L))];
...
function(S)
  ...
  mults, membership, f, l, m, val, j, i, k, x;
      ...
      for f in LClassReps(d[i]) do
        ...
        if schutz[m] = true then
          ...
          if schutz[m] = false then
            ...
            repeat
              n := n + 1;
              if membership(schutz[m], lambdaperm(reps[m][val][n], f)) then
...
function(L)
  ...
  if not IsGreensClassNC(L) 
  # FIXME this is a hack because of the problems with creating the group
  # generated by the null map matrix. 
    and not ActionRank(Parent(L))(Representative(L)) = 0 then 
